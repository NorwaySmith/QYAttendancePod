	        ...
	        if(operation=="approving_test"){
	        	...
	        	if(value){
	        		if(htmlType=="detail"){
		        		...
		        		html+="<tr>";
		        		if(htmlType=="picture"){
		        			html+="<th style=\"vertical-align:top;line-height:64px;\">"+cnName+"：</th>";
		        		}else{
		        			html+="<th>"+cnName+"：</th>";
		        		}
		        		html+="<td>";
...
function createHead(atime,userName,advice,approveResult,userId,index,totalState,userNum,taskName,operation){
	...
	if(operation=="view" && approveResult == 3){
		var mapParam=getPageParam("flow_detail");
		var processerUser=JSON.parse(mapParam.get("processerUser"));
...
function handle(type){
	...
	setTimeout(function(){
		...
		mapParam.put("formData",formData);
