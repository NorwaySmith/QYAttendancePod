 ...
 */
//显示单选列表
function showRadioHtml(name,cnName,radios,type){
	if(type==1){
		cnName=cnName.substring(0,cnName.lastIndexOf("_"));
	}
	var templateHtml='<div id="'+name+'" class="mui-popover mui-top-zero radioDiv" >';
	templateHtml+='<div class="mui-scroll-wrapper">';
	templateHtml+='<div class="mui-scroll">';
	templateHtml+='<ul class="mui-table-view" style="padding-top:0px;">';
	for(var j=0;j<radios.length;j++){
		templateHtml+='<li class="mui-table-view-cell" name='+name+'>'+radios[j]+'</li>';
	}
	templateHtml+='</ul></div></div></div>';
	return templateHtml;
}
//显示多选列表
function showCheckHtml(name,cnName,radios,type){
	if(type==1){
		cnName=cnName.substring(0,cnName.lastIndexOf("_"));
	}
	var templateHtml='<div id="'+name+'" class="mui-popover mui-top-zero chkDiv">';
	templateHtml+='<div class="mui-scroll-wrapper">';
	templateHtml+='<div class="mui-scroll">';
	templateHtml+='<ul class="mui-table-view check" style="padding-top:0px;">';
	for(var j=0;j<radios.length;j++){
		templateHtml+='<li class="mui-table-view-cell"><span>'+radios[j]+'</span><i class="checkoff mui-pull-right"></i></li>';
	}
	templateHtml+='</ul>';
...
//绑定选择事件
function bindSelectRadio(){
	$("body").delegate("div.radioDiv ul li","tap",function(){
		selectMenu(this);
		return false;
	});
}
//取消绑定选择事件
function unbindSelectRadio(){
	$("body").undelegate("div.radioDiv ul li","tap");
}
//绑定选择事件
function bindSelectChk(){
	$("body").delegate("div.chkDiv ul li","tap",function(){
		selectChk(this);
		return false;
	});
}
//取消绑定选择事件
function unbindSelectChk(){
		...
		if($(this).attr("canEdit")=="false"){
			...
			if($(this).hasClass("calendar")){
				...
				$(this).bind("tap",function(){
					...
					var name=$(this).attr("name");
					//已选择项默认选中
					var defaultValue=$(this).children("span").html();
					$("#"+name+" ul li").removeClass("on");
					if(defaultValue!="请选择"){
						var valueAry=defaultValue.split(",");
						var li=$("#"+name+" ul li");
						for(var i=0;i<valueAry.length;i++){
							for(var j=0;j<li.length;j++){
								var cnName=li.eq(j).children("span").html();
								if(cnName==valueAry[i]){
									li.eq(j).addClass("on");
									break;
								}
			...
			//清理复选框选择
			var chks=document.querySelector("a[name='chk']");
			if(chks){
				chks.addEventListener("tap",function(){
					$("div ul.check li").removeClass("on");
				});
			}
		}
	});
...
function selectMenu(obj){
	...
	$("#formDiv").show();
	$("#formDiv span[name='"+name+"']").html($(obj).html());
	mui("#"+name).popover('toggle');
...
function sureChk(id){
	...
	$("#formDiv span[name='"+id+"']").html(chkCont);
	mui('#'+id).popover('toggle');
	//移除颜色
	removeSelectStyle($("#formDiv span[name='"+id+"']"));
	unbindSelectChk();
}
//计算控件
function caculate(){
	...
	for(var i=0;i<len;i++){
		...
		$(".mx_detail ul.ul_"+parentName).eq(i).find(".applyForm_"+parentName).each(function(j){
			var value="";
	 		if($(this).attr("type")=="text"||$(this).attr("type")=="number"||$(this).is("textarea")){
	 			value=$(this).val();
	 		}else if($(this).attr("picture")=="picture"){
	 			value="###picture***"+$(this).html();
	 		}else{
	 			value=$(this).html();
	 		}
	 		if(value=="请选择"||value=="请选择（必选）"){
