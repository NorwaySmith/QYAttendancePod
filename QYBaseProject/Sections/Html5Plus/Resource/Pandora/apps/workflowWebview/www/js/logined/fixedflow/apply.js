	...
	if(result!=null){
		...
		for(var i=0;i<result.length;i++){
	        ...
	        if(htmlType=="text"){//输入框
	        	...
	        	html+="<li class=\"mui-table-view-cell showSelectLi\">";
	        	html+="<a href=\"javascript:void(0);\" class=\"mui-navigate-right radio\" name="+name+" canEdit="+canEdit+"><label class=\"titleName\" >"+cnName+"</label>";
	        	html+="<span class=\"contBox contSelect applyForm "+showSelectClass+"\" require="+required+" cnName="+cnName+" name="+name+">"+showValue+"</span></a>";
	        	<!--单选项-->
	        	templateHtml=showRadioHtml(name,cnName,radios,0);
	        	...
	        	var radios=result[i].radios;
	        	html+="<li class=\"mui-table-view-cell showSelectLi\">";
	        	html+="<a href=\"javascript:void(0);\"  name="+name+" class=\"mui-navigate-right checkbox\" canEdit="+canEdit+"><label class=\"titleName\" >"+cnName+"</label>";
	        	html+="<span class=\"contBox contSelect applyForm "+showSelectClass+" \" require="+required+" cnName="+cnName+" name="+name+" >"+showValue+"</span></a>";
	        	<!--复选项-->
	        	templateHtml=showCheckHtml(name,cnName,radios,0);
				$("#formDiv").after(templateHtml);
	        	...
	        	html+="<span class=\"contBox contSelect applyForm "+showSelectClass+"\" require="+required+" cnName="+cnName+" name="+name+">"+showValue+"</span></a>";
	        	<!--下拉-->
	        	templateHtml=showRadioHtml(name,cnName,options,0);
				$("#formDiv").after(templateHtml);
	        ...
	        }else if(htmlType=="picture"){
	        	showValue="请选择";
	        	if(required){
	        		showValue+="（必选）";
	        	}
	        	html+="<li class=\"mui-table-view-cell add_pic\">";
	        	html+="<a href='javascript:void(0);' class=\"picture\" canEdit="+canEdit+" name="+name+" ><label class='titleName' >"+cnName+"</label>";
	        	html+="<span class=\"applyForm\" require="+required+" cnName="+cnName+"  name="+name+" style='display:none'></span><em class='camera_btn'></em></a>";
	        	html+="<div class=\"img-group\" name="+name+">";
				html+="</div>";
	        }
			html+="</li>";
		}
	...
	if (details) {
		...
		for (var j = 0; j < details.length; j++) {
	        ...
	        if(htmlType=="text"){//输入框
	        	...
	        	html+="<li class=\"mui-table-view-cell showSelectLi\">";
	        	html+="<a href=\"javascript:void(0);\" class=\"mui-navigate-right radio\" name="+name+" canEdit="+canEdit+"><label class=\"titleName\" >"+showcnName+"</label>";
	        	...
	        	<!--单选项-->
	        	templateHtml=showRadioHtml(name,cnName,radios,1);
				$("#formDiv").after(templateHtml);
	        }else if(htmlType=="checkbox"){//复选
	        	showValue="请选择";
	        	if(defaltValue){
	        	...
	        	var radios=details[j].radios;
	        	html+="<li class=\"mui-table-view-cell showSelectLi\">";
	        	html+="<a href=\"javascript:void(0);\"  name="+name+" class=\"mui-navigate-right checkbox\" canEdit="+canEdit+"><label class=\"titleName\" >"+showcnName+"</label>";
	        	html+="<span class=\"contBox contSelect applyForm_"+parentName+" "+showSelectClass+" \" require="+required+" cnName="+cnName+" name="+name+" >"+showValue+"</span></a>";
	        	<!--复选项-->
	        	templateHtml=showCheckHtml(name,cnName,radios,1);
				$("#formDiv").after(templateHtml);
	        }else if(htmlType=="select"){//下拉
	        	showValue="请选择";
	        	if(defaltValue){
	        	...
	        	html+="<li class=\"mui-table-view-cell showSelectLi\">";
	        	html+="<a href=\"javascript:void(0);\"  class=\"mui-navigate-right radio\" name="+name+" canEdit="+canEdit+"><label>"+showcnName+"</label>";
	        	html+="<span class=\"contBox contSelect applyForm_"+parentName+" "+showSelectClass+"\" require="+required+" cnName="+cnName+" name="+name+">"+showValue+"</span></a>";
	        	<!--下拉-->
	        	templateHtml=showRadioHtml(name,cnName,options,1);
				$("#formDiv").after(templateHtml);
	        }else if(htmlType=="textarea"){
	        	var showValue="";
	        	if(defaltValue){
	        	...
	        	html+="<li class=\"mui-table-view-cell add_pic\">";
	        	html+="<a href='javascript:void(0);' class=\"picture\" canEdit="+canEdit+" name="+name+" ><label class='titleName' >"+showcnName+"</label>";
	        	html+="<span class=\"applyForm_"+parentName+"\" picture='picture' require="+required+" cnName="+cnName+"  name="+name+" style='display:none'></span><em class='camera_btn'></em></a>";
	        	html+="<div class=\"img-group\" name="+name+">";
				html+="</div>";
	        }
			html+="</li>";
...
//获取验证标记 
function getToken(){
		mui.ajax(window.windowCommon.basePath+'token/getToken.action?time='+(new Date()).getTime(),{
		data: {
		...
		success:function(data){
			...
			var submitToken=data.submitToken;
			$("#jsessionid").val(jsessionid);
			$("#submitToken").val(submitToken);
		},
...
function submitApply(obj){
	...
	//获取页面参数
	var mapParam=getPageParam("flow_apply");
	var processId=mapParam.get("processId");
	//下一步操作
	...
	//显示提交等待
	showWaiting("处理中，请等待...");
	var submitURL=window.windowCommon.basePath+'workflow/option!startProcess.action?time='+(new Date()).getTime();
	var jsessionid=$("#jsessionid").val();
	var submitToken=$("#submitToken").val();
	if(jsessionid){
		submitURL+=';jsessionid='+jsessionid;
	}
	mui.ajax(submitURL,{
		data: {
			"processId":processId,  
...
//			console.log(errorThrown);
	...
	function bindCloseIcon(){
		$("body").delegate("div.radioDiv header a[name='iconClose']","tap",function(){
		...
		$("body").delegate("div.chkDiv header a[name='iconClose']","tap",function(){
			var id=$(this).parent().parent().attr("id");
			setTimeout(function(){
				mui("#"+id).popover("toggle"); 
			},300);
