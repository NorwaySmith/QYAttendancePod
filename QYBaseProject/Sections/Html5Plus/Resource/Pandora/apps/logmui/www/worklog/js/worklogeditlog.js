mui.plusReady(function() {
	...
	//处理 已完成内容 输入小于10个字符时，下一步按钮不可点
	/*var nextBtn = document.getElementById("next");
	$("#next").addClass("noclick");		
	var successContentBtn = document.getElementById("successContent");
	successContentBtn.addEventListener("input", function() {
		var successContent = $("#successContent").val();
		if (null == successContent || "" == successContent || "" == $.trim(successContent) || $.trim(successContent).length<10){
			$("#next").addClass("noclick");			
				...
				if (msg.indexOf("100||") == 0) {
					if (msg.split("||")[1] == 1) {
						...
						mui.openWindow({
							...
							id: 'worklogrelease',
							extras:{
						    	indexId : plus.webview.currentWebview().indexId,//首页ID
...
function createUpload(filePath, id, fileName) {
		...
		// 上传状态 0 上传中 1完成 2取消
		taskObj.status = 0;
		console.log(window.windowCommon.basePath + "worklog/upload.c?_clientType=wap&fileId=" + id)
		//上传
