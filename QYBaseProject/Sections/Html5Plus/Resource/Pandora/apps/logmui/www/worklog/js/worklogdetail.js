...
mui.init({
	...
	beforeback: function(){//关闭前触发方法
		//刷新来源页列表
		plus.webview.getWebviewById(fromPage).evalJS("refrenceList("+logid+","+praiseNum+","+commentNum+")");
	}
});
mui.plusReady(function() {
	...
	var logUserId = self.logUserId;//日志发布人
...
var clickPraise = function(worklogId) {
	...
	console.log('点赞/取消赞操作');

	//var worklogId = self.logid;
	...
	mui.ajax(url, {
		...
		success: function(response) {
			if (response.code != 0) {
				...
				if (response.num > 0) {
					console.log("response.num1" + (response.num > 0));
					var goods = mui(".goods-user")[0];
					goods.addEventListener("tap", openWorkloggoods);
				} else {
					console.log("response.num false");
					var goods = mui(".goods-user")[0];
	...
	mui.ajax(window.windowCommon.basePath + 'worklog/getLogInfo.c', {
		...
		success: function(msg) {
			...
			if (msg.indexOf("100||") == 0) {
				...
				message += "<span class=\"state\"><i class=\"state-1\"></i>已完成</span> " + json.successContent;
				message += "</li>";
				message += "<li style=\"word-break:break-all\">";
				message += "<span class=\"state\"><i class=\"state-2\"></i>未完成</span> " + json.failContent;
				message += "</li>";
				message += "<li style=\"word-break:break-all\">";
				message += "<span class=\"state\"><i class=\"state-3\"></i>需协调</span> " + json.tieContent;
				message += "</li>";
				message += "<li style=\"word-break:break-all\">";
