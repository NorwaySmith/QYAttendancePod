 ...
 */
var memberId;
var seriesContactData = new Array();
var seriesCustomerData = new Array();
...
mui.plusReady(function() {
	...
	if(plus.webview.currentWebview().userName != undefined && plus.webview.currentWebview().userName != null){
		mui('.mui-title')[0].innerHTML = plus.webview.currentWebview().userName + '的客户统计报表';
...
function fullStatisticsInfo() {
	...
	customerStatisticalReportAPI._paycustomerStatisticalReport(param, function(response) {
		//console.log(JSON.stringify(response));
		$("#statisticsMsg").text("全年新增客户"+response.customer.Customer+"个,新增联系人"+response.contact.Contact+"个");
			seriesContactData = response.contact.ContactData;
			seriesCustomerData = response.customer.CustomerData;
	...
	subli03.addEventListener('tap', function() {
		mui.openWindow({
		    ...
		    extras:{
		    	...
		    	tourl:"../statistics/customerStatisticalReport.html",
		    	toviewid:'customerStatisticalReport.html',
		    }
		});
	});
