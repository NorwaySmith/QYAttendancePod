...
@property(nonatomic,copy)NSString *method;//pc传过来的方法名
/**
 *  背景点击事件
 */
...
-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isHaveNavControler:(BOOL)isHaveNavControler method:(NSString *)method{
    _method=method;
...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    ...
    self.selfFrame = CGRectMake(toolViewX, toolViewY, toolViewW, toolViewH);
}
-(void)setUpPickView{
...
-(void)remove{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.frame=CGRectMake(CGRectGetMinX(self.frame), CGRectGetHeight(window.frame), CGRectGetWidth(self.frame), CGRectGetHeight(self.frame));
...
-(void)show{
    UIWindow *window=[UIApplication sharedApplication].keyWindow;
    self.control = [[UIControl alloc] initWithFrame:window.frame];
    [_control addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    [window addSubview:_control];
    
    self.frame=CGRectMake(CGRectGetMinX(_selfFrame), CGRectGetHeight(window.frame), CGRectGetWidth(_selfFrame), CGRectGetHeight(_selfFrame));
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        NSString *dateValueString = [dateFormatter stringFromDate:_datePicker.date];
        NSDate *dateValue1 = [dateFormatter dateFromString:dateValueString];
        int time=[dateValue1 timeIntervalSinceDate:nowDate];
        if (time < 0) {
            [self alerts:@"截止时间必须大于当前时间!" ];
            return;
        }
        _resultString=[dateFormatter stringFromDate:_datePicker.date];
//        _resultString=[NSString stringWithFormat:@"%@",_datePicker.date];
    }
    if ([self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:method:)]) {
        [self.delegate toobarDonBtnHaveClick:self resultString:_resultString method:_method];
    }
    [self remove];
}
-(void)alerts:(NSString*)alertString{
    ...
    [alertHud hide:YES afterDelay:2];
