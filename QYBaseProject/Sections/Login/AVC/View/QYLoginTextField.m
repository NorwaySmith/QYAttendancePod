...
@property (nonatomic, strong) NSString *leftViewString;
...
- (instancetype)initWithLeftViewImage:(NSString *)leftString
{
    self = [self init];
    if (self){
        self.leftViewString = leftString;
        _leftTextLabel =[[UILabel alloc] init];
        _leftTextLabel.textAlignment=NSTextAlignmentLeft;
        _leftTextLabel.frame=CGRectMake(0, 0, 80, 40);
        if ([leftString isEqualToString:@"密不码"]) {
            NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:leftString];
            [attributeString addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(1, 1)];
            _leftTextLabel.attributedText=attributeString;
        }else{
            _leftTextLabel.text=leftString;
        }
        self.leftView = _leftTextLabel;
...
- (void)layoutSubviews
{
    ...
    {
        self.secureTextEntryToggle.frame =
        CGRectIntegral(CGRectMake(CGRectGetWidth(self.bounds) - CGRectGetWidth(self.secureTextEntryToggle.frame),(CGRectGetHeight(self.bounds) - CGRectGetHeight(self.secureTextEntryToggle.frame)) / 2.0,CGRectGetWidth(self.secureTextEntryToggle.frame),CGRectGetHeight(self.secureTextEntryToggle.frame)));
...
- (CGRect)calculateTextRectForBounds:(CGRect)bounds
{
    ...
    if (_leftViewString)
    {
        CGFloat leftViewWidth = _leftTextLabel.frame.size.width;
...
- (CGRect)leftViewRectForBounds:(CGRect)bounds
{
    if (_leftViewString)
    {
        return CGRectIntegral(CGRectMake(_textInsets.left, (CGRectGetHeight(bounds) - _leftTextLabel.frame.size.height) / 2.0, _leftTextLabel.frame.size.width, _leftTextLabel.frame.size.height));
    }
