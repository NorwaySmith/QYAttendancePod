...
#define RGBColor(r,j,b) [UIColor colorWithRed:r/255.0 green:j/255.0 blue:b/255.0 alpha:1.0]//短信通模版
...
- (void)setup {
	...
	tokenField = [[TITokenField alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 44)];
	...
	separator = [[UIView alloc] initWithFrame:CGRectMake(0, tokenFieldBottom, self.bounds.size.width, 0.5)];
	[separator setBackgroundColor:[UIColor colorWithRed:217/255.0 green:217/255.0 blue:217/255.0 alpha:1.0]];
...
- (void)setup
{
    ...
    self.showLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.frame)+5, CGRectGetMinY(self.frame), 280, CGRectGetHeight(self.frame))];
    ...
    self.showLabel.lineBreakMode = NSLineBreakByTruncatingMiddle;
...
- (void)didBeginEditing
{
   self.placeholderLabel1.hidden = YES;
    self.showLabel.text = kTextEmpty;
    self.showLabel.hidden = YES;
...
//          self.showLabel.frame=CGRectMake(CGRectGetMinX(self.frame)+5, CGRectGetMinY(self.frame)-15, 285, CGRectGetHeight(self.frame));

			if (tokens.count > 1 && untokSize.width > availableWidth){
//				untokenized = [NSString stringWithFormat:@"共%d人", titles.count];
                self.showLabel.text=[NSString stringWithFormat:@"%@  等%d人",untokenized,titles.count];
			}
            else
            {
                self.showLabel.text = untokenized;
            }
			[titles release];
...
{
	self.placeholderLabel1 = [[[UILabel alloc] initWithFrame:CGRectMake(10, 0, 200, 40)] autorelease];
    self.placeholderLabel1.backgroundColor=[UIColor clearColor];
    [self.placeholderLabel1 setTextColor:RGBColor(187, 187, 187)];
    [self.placeholderLabel1 setFont:[UIFont systemFontOfSize:(self.font.pointSize + 3)]];
    self.placeholderLabel1.hidden = YES;
    [self addSubview:self.placeholderLabel1];
    
    if (placeholder)
    {
        self.placeholderLabel1.hidden = NO;
		self.placeholderLabel1.text = placeholder;
    }
	else
	{
...
- (void)dealloc
{
    ...
    [self.placeholderLabel1 release];
    [super dealloc];
}
...
+ (UIColor *)greenTintColor {
     return [UIColor colorWithRed:137.0/255.0 green:147.0/255.0 blue:156.0/255.0 alpha:1];
//	return [UIColor colorWithRed:0.333 green:0.741 blue:0.235 alpha:1];
////    rgb(199,247,183)
//    RGB Decimal 50, 248, 0
//
//    rgb(50,248,0)
    
//   return [UIColor colorWithRed:137.0/255.0 green:147.0/255.0 blue:156.0/255.0 alpha:1];
}
+ (UIColor *)hoaryTintColor{
 return [UIColor colorWithRed:137.0/255.0 green:147.0/255.0 blue:156.0/255.0 alpha:1];
}
#pragma mark Layout
- (void)sizeToFit {
...
//	CGFloat nonHighlightedComp[8] = {red, green, blue, 0.5, red, green, blue, 0.7};
